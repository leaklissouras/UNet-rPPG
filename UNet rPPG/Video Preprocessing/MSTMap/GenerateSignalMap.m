function SignalMap = GenerateSignalMap(SignalMap, img, idx, lmks, lmk_num)
    ...
    %Scale the Landmarks to match image size; landmarks are based off of
    %600 width
    ...
    if lmk_num == 81
        ...
        %NEW
        ROI_cheek_left1 = [3 4 5 48 31 3] + 1; %FIXED
        ROI_cheek_left2 = [1 2 3 30 29 28 27 1] + 1; %FIXED
        ROI_cheek_right1 = [13 12 11 54 35 13] + 1; %FIXED
        ROI_cheek_right2 = [15 14 13 30 29 28 27 15] + 1; %FIXED
        ROI_mouth = [5 48 59 58 57 56 55 11 10 9 8 7 6 5] + 1; %FIXED BUT DIFF LENGTH
        ROI_forehead = [18 19 20 21 27 22 23 24 25 80 71 70 18] + 1; %FIXED BUT DIFF LENGTH
        
        %forehead = lmks(:,ROI_forehead);
        %eye_distance = norm(lmks(:,1) - lmks(:,10)); %FIX THIS
        %tmp = (mean(lmks(:,19:26)')+ mean(lmks(:,27:34)'))/2 - (mean(lmks(:,1:9)')+mean(lmks(:,10:18)'))/2; %FIX THIS
    ...
    mask_ROI_forehead = poly2mask(lmks(1,ROI_forehead),lmks(2,ROI_forehead),size(img,1), size(img,2));
    %% plot ROI
    figure %(1);
    clf;
    %imshow(img(:,:,1:3));
    ...
    plot(lmks(1,ROI_forehead),lmks(2,ROI_forehead), 'Linewidth',2)
